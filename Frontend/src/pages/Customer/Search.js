...
class Search extends Component {
  ...
  render() {
    ...
    const modeFilter = this.state.restaurants.filter((res) => {
      const delivery = (res.operationalMode === this.state.operationalMode || res.operationalMode === "Pickup and Delivery") 
      return (delivery);
    });
    ...
    const cityFilter = dietFilter.filter((res)=>{
      if (this.state.givenCity){
        const city = (res.city).trim().toLowerCase()===(this.state.givenCity).trim().toLowerCase()
        return city;
      }
      else{
        return (true)
    ...
    const searchFilter = cityFilter.filter((res)=>{
      ...
      var isCuisine = false;
      if (res.dish_list){
        const dish_ind = (res.dish_list).findIndex(
          element => element.trim().toLowerCase().includes((this.state.searchInput).trim().toLowerCase()))
        isDish = (dish_ind!==-1)?true:false
      }
      if (res.cuisine){
        isCuisine = (res.cuisine).trim().toLowerCase() === (this.state.searchInput).trim().toLowerCase()
      }

      const search = isCuisine || isDish || (res.name).trim().toLowerCase()===(this.state.searchInput).trim().toLowerCase()
    ...
    const resList = <AllRestaurants restaurantsList={searchFilter} role={this.state.role} favourites={this.state.favourites}/>;
    ...
    return (
      ...
      <div style={{paddingLeft:6}}>
        ...
        <div className="col-lg-1 col-md-4">
              ...
              <input type="checkbox" id="feature1" value="Vegan" onChange={this.dietChangeHandler}/>
              <div>
              <span><GiSolidLeaf/> Vegan</span>
                </div>
              </article>
              <article class="feature2">
              <input type="checkbox" id="feature2" value="Vegetarian" onChange={this.dietChangeHandler}/>
              <div>
              <span><GiPlantsAndAnimals/> Vegetarian</span>
                </div>
              </article>
              <article class="feature3">
              <input type="checkbox" id="feature3" value="Non-Veg" onChange={this.dietChangeHandler}/>
              <div>
              <span><GiMeat/> Non-Veg</span>
                </div>
              </article>
              </div>
